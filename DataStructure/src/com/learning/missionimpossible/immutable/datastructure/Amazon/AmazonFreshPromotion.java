    ...
    public static int winPrize(String [][] codeList,String [] shoppingCart){
        ...
        while(i<codeList.length && j+codeList[i].length<=shoppingCart.length){
            ...
            for(int k=0;k<codeList[i].length;k++){
                ...
                if(!codeList[i][k].equalsIgnoreCase(shoppingCart[j+k]) && !codeList[i][k].equals("anything")) {
                    match = false;
                    break;
                }
            }
            ...
            if(match==true) {
// if there is match then we need to match shooping cart from j postion which will sum of previous j position
                // codelist length
                j += codeList[i].length;
                i++;
            }
